...
def duplicate_and_unique_movies(dataset, index_):
    ...
    duplicate = []
    for i in dataset:
        movies_name.append(i[index_])
    for i in movies_name:
        if(movies_name.count(i) > 1):
...
def rate_bucket(dataset, rate_low, rate_high):
    ...
    rated_movies = []
    for i in dataset:
        if not (float(i[-4]) >= rate_low and float(i[-4]) < rate_high):
...
reviews_max ={}
for i in movies:
    key_list = list(reviews_max.keys())
    if(i[-2] in key_list):
        if(int(reviews_max[i[-2]]) > int(i[-3])):
            reviews_max[i[-2]] = reviews_max[i[-2]]
        elif(int(reviews_max[i[-2]]) < int(i[-3])):
            reviews_max[i[-2]] = i[-3]
    else:
        reviews_max[i[-2]]=i[-3]
...
movies_clean = list(reviews_max.keys())
review_max = list(reviews_max.values())
...
high_rated_movies = rate_bucket(movies_en,0,8)
